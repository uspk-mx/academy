...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  return {
    items,
    reviewableCourses,
    // TODO(hygraph): swap for a StudentReviewsPage model, same pattern as the
    // marketing loaders (defaults keep the page working meanwhile).
    labels: defaultReviewsPageLabels,
  }
}
...
export async function action({ request }: Route.ActionArgs) {
  ...
  try {
    if (intent === "delete") {
      ...
      return data<ReviewActionData>({ ok: true }, { headers: cookies(setCookies) })
    }
    ...
    if (message.includes("already reviewed")) return fail("alreadyReviewed", 409)
